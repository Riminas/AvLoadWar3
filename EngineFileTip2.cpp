...
EngineFileTip2::EngineFileTip2(sf::RenderWindow& t_Window, sf::Font& t_font, DataPath& t_DataPath, DataMaps& t_DataMaps, OptionsData& t_OptionsData)
    : m_Window(t_Window), font(t_font), m_DataPath(t_DataPath), m_DataMaps(t_DataMaps), m_OptionsData(t_OptionsData)
{
}
...
bool EngineFileTip2::engineFile() {
    ...
    if (fileName == L"Error") return false;

    std::wstring str2 = m_DataPath.getSavePath();
    m_DataMaps.NameMaps(fileName);
    if (!m_DataMaps.PutSaveCode(str2)) {
        ...
        bool faic = false;
        NewDirectory app(m_Window, font, newPosition, wstr2, wstr, faic, m_OptionsData);
        app.newDirectory();
        return false;
    }
...
void EngineFileTip2::activeGameFalse(const HWND& hWndWindow) {
    // Implement as needed
}
